...
def model(batchdata,learning_rate=0.001, num_epochs=25000, Train=True):
    """
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 1000 == 0:
                ...
                # correct_prediction = tf.equal(tf.argmax(Z3), Y)
        ...
        Z3_prob = tf.nn.softmax(Z3)
        out_prob = np.max(sess.run(Z3_prob,feed_dict={X: batchdata[0]['xtest'].transpose(),Y: batchdata[0]['ytest']}),axis=0)
        # plt.plot(out_prob)
        # plt.show()
        print ("Parameters have been trained!")

        for timestep in range(1,len(batchdata),1):
            minibatch_cost = sess.run(cost,feed_dict={X: batchdata[timestep]['xtest'].transpose(),Y: batchdata[timestep]['ytest']})
            costs.append(minibatch_cost)
